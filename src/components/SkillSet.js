...
export default function SkillSet({ skillName, percentage, detailSkills }) {
    ...
    return (
        <div className='w-1/5 min-w-[150px] max-[785px]:w-1/3 max-[420px]:w-full flex flex-col items-start gap-2'>
            ...
            <button className={'ml-[70%] max-[420px]:ml-[85%] border border-green-500 rounded-full text-[10px] font-semibold p-1 ' + (toggle ? '   bg-white text-green-500 hover:bg-green-500 hover:text-white' : 'bg-green-500 text-white hover:bg-white hover:text-green-500')} onClick={() => { setToggle(state => !state) }}>
